...
$.extend(PopUpCal.prototype, {
	...
	_checkExternalClick: function(event) {
		...
		var rangeElement = $("#pickerDemoForm").find('a.rangeDisplay');
		var parentDiv = rangeElement.next();
		var picker1 = parentDiv.find('.calendar_div:eq(0)');
		var picker2 = parentDiv.find('.calendar_div:eq(1)');
		picker1.fadeOut(0);
		picker2.fadeOut(0);
		parentDiv.find('h2.selectDate, a.dateRange_close').fadeOut(0);
		$("#pickerDemoForm").find('.dateRange:eq(0)').slideUp(200);
...
$.fn.dateRangePicker = function(options){
		...
		var url = options.url;
		var presetMenu = {
				pastRange: '<div class="dateRange_contain dateRange_wide"><div class="dateRange" style="display:none"><ul class="dateRange_presets">'+
					'<li class="dateRange_last30" style="display:none"><a href="javascript://">Past 30 Days</a></li>'+
					'<li class="dateRange_lastYear" style="display:none"><a href="javascript://">Past 12 Months</a></li>'+
					'<li class="dateRange_yearToDate" style="display:none"><a href="javascript://">Current YTD</a></li>'+
					'<li class="dateRange_allDates" style="display:none"><a href="javascript://">All Dates</a></li>'+
					'<li class="dateRange_allBefore" style="display:none"><a href="javascript://">All Dates Before...</a></li>'+
					'<li class="dateRange_allAfter" style="display:none"><a href="javascript://">All Dates After...</a></li>'+
					'<li class="dateRange_specificDate" style="display:none"><a href="javascript://">Specific Date...</a></li>'+
					'<li class="dateRange_dateRange dateRange" style="display:none"><a href="javascript://">Date Range...</a></li>'+
					'</ul>'+
		...
		$('.dateRange_close:first').click(function(){
		...
		rangeElement.click(function(){
			...
			var both_range = $('.dateRange_dateRange:first');
			$(both_range).showPickers('both', 'Select a Date Range:');
			$('.dateRange:first').css('overflow','hidden');
			return false;
		});
		...
		var dates = {
			...
			//if we already have the values
			var from = dates.today();
			var to = dates.today();
			
			if(getQuerystring('from') != null && getQuerystring('from') != ''){
				from = getQuerystring('from');
				from = from.replace(/-/g,"/");
			}
				
			if (getQuerystring('to') != null && getQuerystring('to') != '') {
			...
			$.fn.showPickers = function(showPicker, pickerMsg){
				...
				picker1.css('left', '5px');
				picker2.css('left', '215px');
				$('.dateRange:first').find('h2.selectDate').css('top','15px');
				$('.dateRange:first').find('h2.selectDate').css('font-size','13px');
				$('.dateRange:first').find('h2.selectDate').css('left','8px');
				$('.dateRange:first').find('h2.selectDate').css('left','8px');
				$('.dateRange:first').find('h2.selectDate').css('width','200px');
				$('.dateRange:first').find('h2.selectDate').css('background','#F9F9F9');
				$('.dateRange:first').find('a.dateRange_close').css('width','30px');
				$('.dateRange:first').find('a.dateRange_close').css('left','361px');
				$('.dateRange:first').find('a.dateRange_close').css('display','block');
				$('.dateRange:first').css('left','-176px');
				$('.dateRange:first').css('height','250px');
				
				//rangeElement.css("width","38.5em");
				...
				$(this).parents('.dateRange:eq(0)').find('ul:eq(0)').animate({height: expandedHeight}, 500,"swing");
					...
					$('.dateRange:first').css('width','39em');
					$('.dateRange:first').css('overflow','hidden');
			}
